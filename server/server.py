...
def proxy():
    ...
    if "file0" in request.files:
        ...
        for key in request.files:
            ...
            pathlib.Path(f"{path}/{uploaded_file_dir}").mkdir(
    ...
    return Response(
        response=res.content, status=res.status_code, headers=dict(res.headers)
    )
...
def check_connection():
    ...
    try:
        res = requests.post(
            f"{url}/function/{FUNCTION}",
            data=json.dumps({"command": 914}).encode("UTF-8"),
            timeout=3,
        )
...
def register_script(message):
    ...
    if url is None:
        url = "None"
    with T_LOCK:
        CONNECTED_CLIENTS[client]["events"]["script"] = {
            f"{project_name}_{script_name}": {
                "project_name": project_name,
                "script_name": script_name,
                "test_ids": test_ids,
            }
    ...
    with T_LOCK:
        if client in CONNECTED_CLIENTS:
            if "script" in CONNECTED_CLIENTS[client]["events"]:
                if (
                    test_id
                    in CONNECTED_CLIENTS[client]["events"]["script"][
                        f"{project_name}_{script_name}"
                    ]["test_ids"]
                ):
                    CONNECTED_CLIENTS[client]["events"]["script"][
...
def openfass_socket():
    ...
    with OPENFAAS_T_LOCK:
        if OPENFAAS_T is None:
            OPENFAAS_T = socketio.start_background_task(
                target=check_openfaas_thread
            )  # Thread(target=check_openfaas_thread)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-h", "--help", action="help", default=argparse.SUPPRESS, help="help"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--extern",
        action="store_true",
        help="use if OpenFaaS is running on the external ip address of your machine",
    )
    parser.add_argument(
        "-l",
        "--local",
        action="store_true",
        help="use if you dont want to use an OpenFaaS server. server will run on 0.0.0.0:80 with no OpenFaaS server",
    )
    parser.add_argument(
        "-r",
        "--redis",
        action="store_true",
        help="use redis (cache). recommended if you dont have SSD",
    )
    parser.add_argument(
        "-rh", "--redishost", help="redis host, default: localhost", metavar=""
    )
    parser.add_argument(
        "-rp", "--redisport", help="redis port, default: 6379", metavar=""
    )
    parser.add_argument(
        "-re",
        "--redisexpire",
        help="redis (cache) expiration timer, default: 600 seconds",
        metavar="",
    )
    parser.add_argument(
        "-rd", "--redisdatabase", help="redis database: 0 - 15, default: 0", metavar=""
    )
    requiredNamed = parser.add_argument_group("required arguments")
    requiredNamed.add_argument(
        "-s", "--host", help="server host, default: 0.0.0.0", metavar=""
    )
    requiredNamed.add_argument(
        "-p", "--port", help="server port, default: 80", metavar=""
    )
    ...
    requiredNamed.add_argument(
        ...
        "--direct",
        action="store_true",
        help="can the browser connect to OpenFaaS directly?",
    )
    ...
    if url is None and extern is False and LOCAL is False:
        print(
            "please provide an openfaasurl using -u or -e or use -l if you dont want to use an openfaas server"
        )
        exit()
    ...
    if extern is True:
        if platform.system() == "Linux":
            host = (
                subprocess.Popen(
                    "echo $(/sbin/ip -o -4 addr list  | awk '{print $4}' | cut -d/ -f1)",
                    shell=True,
    ...
    if use_redis is True:
        ...
        try:
            ...
            print(
                "could not connect to redis. please check your redis server and try again"
            )
            exit()
        handler.REDIS = redis
        handler.EXPIRE = redis_expire
        print(
            f"using redis on {redis_host}:{redis_port} | database: [{redis_database}] | cache lifetime: {redis_expire} seconds"
        )
